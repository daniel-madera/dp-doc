...
\begin{itemize}
\item {\texttt{DOC} -- podadresář \texttt{PDF} obsahuje zadání a kompletní text BP ve formátu PDF,
    podadresář \verb|src| pak {\LaTeX}ové \uv{zdrojové kódy} a~obrázky použité
    v~textu BP.}
\item {\texttt{SRC} -- podadresář \texttt{APP} obsahuje zdrojové kódy webové aplikce včetně souboru \texttt{README} popisující nutná nastavení před spuštěním aplikace.
     podadresář \texttt{SQL} obsahuje zdrojové kódy SQL dotazů pro vložení struktury databáze a výchozích dat.}
\end{itemize}

...
\thispagestyle{plain}
\section*{Poděkování}
Rád bych poděkoval vedoucí Ing.~Janě Vitvarové, Ph.D. za trpělivost a velice užitečné rady v průběhu tvorby této práce. Rovněž bych rád poděkoval učitelce gymnázia Mgr.~Ivaně Bucharové za cenné komentáře k aplikaci a textové části práce.
\thispagestyle{empty}

\newpage
\thispagestyle{plain}
\section*{Abstrakt}
Tato práce se zabývá problematikou učení slovní zásoby cizího jazyka. V rámci práce byla vytvořena aplikace v podobě webové služby jako učební nástroj, který má za úkol pomoci žákům s procvičením slov a k jejich permanentnímu zapamatování. Práce analyzuje a implementuje metody procvičení a připomínání slovíček za použití algoritmů rozloženého opakování. Výstupem práce je aplikace typu SPA v JavaScriptu s podporou knihovny React a webové API v jazyce Python za použití Django frameworku. 

\section*{Klíčová slova}
slovníček, učení, rozložené opakování, webová aplikace, SPA, React, Django

\section*{Abstract}
This thesis describes the issues of learning foreign language vocabulary. During the thesis was developed a Web application as an educational tool that aims to help students with foreign words and help them sustain permanent retention. The thesis analyzes and implements methods of practicing and recalling vocabulary using algorithms based on methods of spaced repetition. The outputs of thesis are a SPA written in JavaScript supported by the React library and a web API in Python language using the Django framework. 

\section*{Keywords}
vocabulary, learning, spaced repetition, web application, SPA, React, Django
...
\section{Úvod}
    ...
    Cílem diplomové práce je provést analýzu, jak zefektivnit učení a motivovat k procvičování slovíček z cizího jazyka. Na základě zjištěných poznatků navrhnout a implementovat aplikaci, která umožní žákům základních škol rozvíjet cizojazyčnou slovní zásobu a vyučujícím poskytuje nástroj pro správu a prezentaci slovíček pro žáky.

    Úvodem se práce zabývá rešerší existujících řešení podobných aplikací. Analýza rovněž zahrnuje shrnutí informací o rozvoji slovní zásoby cizího jazyka v podobě dostupných metod a technik k motivaci učení a k efektivnímu zapamatování slov.

    Na základě zjištěných informací z rešerše byla provedena specifikace požadavků, které sloužily jako hlavní podklad pro návrh aplikace popsaný v kapitole 2. V této kapitole jsou rovněž detailněji popsány způsoby generování testovací sady, průběh importu slov a vybrané metody zajišťující motivaci žáků.

    V kapitole 3 a 4 se práce zabývá implementací navržené slovníkové aplikace v podobě webové služby. Kapitola 3 se věnuje klientské části a kapitola 4 té serverové. V obou jsou představeny technologie, které jsou využívány pro vývoj, jejich implementace a průběh testování. 
...
\section{Serverová část}
    Serverová část aplikace je v podstatě aplikační rozhraní, které poskytuje data ve formátu \gls{json} pro klientskou aplikaci. \gls{api} je napsané v programovacím jazyce Python s využitím Django webového frameworku a Django REST frameworku.
